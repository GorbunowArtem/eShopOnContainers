		...
		public async Task Handle(OrderStatusChangedToAwaitingCouponValidationIntegrationEvent @event)
		{
			...
			using (LogContext.PushProperty("IntegrationEventContext", $"{@event.Id}-Coupon.API"))
			{
				...
				Log.Information(
					...
					couponIntegrationEvent.Id, "Coupon.API", couponIntegrationEvent);

				_eventBus.Publish(couponIntegrationEvent);
			}
		}

		private async Task<IntegrationEvent> ProcessIntegrationEventAsync(
			OrderStatusChangedToAwaitingCouponValidationIntegrationEvent integrationEvent)
		{
